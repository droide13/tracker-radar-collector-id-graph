...
const { SCREENSHOT_REQUESTED, SCREENSHOT_TAKEN, SCREENSHOT_ERR } = require('../helpers/collectorEvents');
 ...
 * @property {string} data - base64 encoded jpeg
 * @property {number} timestamp - unix timestamp in ms
 * @property {string} label - human readable label for the screenshot
 */
...
class ScreenshotCollector extends BaseCollector {
    ...
    init(options) {
        ...
        this.bus.on(SCREENSHOT_REQUESTED, (label) => this._takeScreenshot(label));
    }
     ...
     */
    postLoad() {
        return this._takeScreenshot('post-load');
    }
     ...
     * @returns {Promise<void>}
     */
    async _takeScreenshot(label = 'unlabeled') {
        try {
            ...
            this.bus.emit(SCREENSHOT_TAKEN);
        } catch (e) {
     ...
     */
    async getData() {
        await this._takeScreenshot('final');
